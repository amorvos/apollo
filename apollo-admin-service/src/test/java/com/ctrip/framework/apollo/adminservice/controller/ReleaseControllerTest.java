...
public class ReleaseControllerTest extends AbstractControllerTest {
	...
	public void testReleaseBuild() {
		...
		HttpEntity<MultiValueMap<String, String>> entity = new HttpEntity<MultiValueMap<String, String>>(parameters,
				headers);
		ResponseEntity<ReleaseDTO> response = restTemplate
				.postForEntity(
	...
	public void testMessageSendAfterBuildRelease() throws Exception {
		...
		verify(someMessageSender, times(1)).sendMessage(
				Joiner.on(ConfigConsts.CLUSTER_NAMESPACE_SEPARATOR).join(someAppId, someCluster, someNamespaceName),
