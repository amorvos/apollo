...
public abstract class LoggingCustomizer implements InitializingBean {
	...
	private static boolean cLoggingAppenderPresent = ClassUtils.isPresent(cLoggingAppenderClass,
			LoggingCustomizer.class.getClassLoader());
	...
	private void tryConfigCLogging() throws Exception {
		...
		ReflectionUtils.findMethod(clazz, "setServerPort", int.class).invoke(cLoggingAppender,
				Integer.parseInt(cloggingPort()));
