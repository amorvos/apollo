...
public class CtripUserServiceTest extends AbstractUnitTest {
	...
	public void setUp() throws Exception {
		...
		someResponseType = (ParameterizedTypeReference<Map<String, List<CtripUserService.UserServiceResponse>>>) ReflectionTestUtils
	...
	public void testAssembleSearchUserRequest() throws Exception {
		...
		CtripUserService.UserServiceRequest request = ctripUserService.assembleSearchUserRequest(someKeyword,
				someOffset, someLimit);
	...
	public void testSearchUsers() throws Exception {
		...
		CtripUserService.UserServiceResponse someUserResponse = assembleUserServiceResponse(someUserId, someName,
				someEmail);

		CtripUserService.UserServiceResponse anotherUserResponse = assembleUserServiceResponse(anotherUserId,
				anotherName, anotherEmail);

		Map<String, List<CtripUserService.UserServiceResponse>> resultMap = ImmutableMap.of("result",
				Lists.newArrayList(someUserResponse, anotherUserResponse));
		ResponseEntity<Map<String, List<CtripUserService.UserServiceResponse>>> someResponse = new ResponseEntity<>(
	...
	public void testSearchUsersWithError() throws Exception {
		when(restTemplate.exchange(eq(someUserServiceUrl), eq(HttpMethod.POST), any(HttpEntity.class),
				eq(someResponseType))).thenThrow(new HttpClientErrorException(HttpStatus.INTERNAL_SERVER_ERROR));
	...
	public void testFindByUserId() throws Exception {
		...
		Map<String, List<CtripUserService.UserServiceResponse>> resultMap = ImmutableMap.of("result",
				Lists.newArrayList(someUserResponse));
		ResponseEntity<Map<String, List<CtripUserService.UserServiceResponse>>> someResponse = new ResponseEntity<>(
				resultMap, HttpStatus.OK);
	...
	private void compareUserInfoAndUserServiceResponse(CtripUserService.UserServiceResponse userServiceReponse,
			UserInfo userInfo) {
	...
	private CtripUserService.UserServiceResponse assembleUserServiceResponse(String userId, String name, String email) {
