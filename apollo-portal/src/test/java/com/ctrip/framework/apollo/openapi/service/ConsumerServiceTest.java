...
public class ConsumerServiceTest extends AbstractUnitTest {
	...
	public void testGetConsumerIdWithNullToken() throws Exception {
		...
		verify(consumerTokenRepository, never()).findTopByTokenAndExpiresAfter(anyString(), any(Date.class));
	}
	...
	public void testGenerateConsumerToken() throws Exception {
		...
		assertEquals("d0da35292dd5079eeb73cc3a5f7c0759afabd806",
	...
	public void testAssignNamespaceRoleToConsumer() {
		...
		doReturn(namespaceModifyConsumerRole).when(consumerService).createConsumerRole(consumerId, modifyRoleId,
				testOwner);
		doReturn(namespaceReleaseConsumerRole).when(consumerService).createConsumerRole(consumerId, releaseRoleId,
				testOwner);
