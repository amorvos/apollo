...
public class NamespaceServiceTest extends AbstractUnitTest {
	...
	public void testDeleteNamespaceBranchHasInstance() {
		...
		when(branchService.findBranchBaseInfo(testAppId, testEnv, testClusterName, testNamespaceName))
				.thenReturn(branch);
		when(instanceService.getInstanceCountByNamepsace(testAppId, testEnv, branchName, testNamespaceName))
				.thenReturn(10);
	...
	public void testDeleteEmptyNamespace() {
		...
		verify(namespaceAPI, times(1)).deleteNamespace(testEnv, testAppId, testClusterName, testNamespaceName,
				operator);
