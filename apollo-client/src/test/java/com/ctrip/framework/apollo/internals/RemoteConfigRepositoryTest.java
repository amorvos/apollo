...
public class RemoteConfigRepositoryTest {
	...
	public void testAssembleQueryConfigUrl() throws Exception {
		...
		String queryConfigUrl = remoteConfigRepository.assembleQueryConfigUrl(someUri, someAppId, someCluster,
				someNamespace, null, notificationMessages, someApolloConfig);
		...
		assertTrue(queryConfigUrl.contains(
				"messages=" + UrlEscapers.urlFormParameterEscaper().escape(gson.toJson(notificationMessages))));
	}
	...
	private ApolloConfig assembleApolloConfig(Map<String, String> configurations) {
		...
		ApolloConfig apolloConfig = new ApolloConfig(someAppId, someClusterName, someNamespace, someReleaseKey);
	...
	public static class MockHttpUtil extends HttpUtil {
		...
		public <T> HttpResponse<T> doGet(HttpRequest httpRequest, Class<T> responseType) {
			...
			throw new ApolloConfigException(
					String.format("Http request failed due to status code: %d", someResponse.getStatusCode()));
		}
